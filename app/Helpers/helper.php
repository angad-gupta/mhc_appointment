...
function setSelectOption($find, $local)
{
    if ($find == $local) {
        return 'selected';
    }
...
{
    $numberToWord = new \NumberToWords\NumberToWords();
    $numberTransformer = $numberToWord->getNumberTransformer('en');
    return $numberTransformer->toWords($number);
}

function authUserRoleName()
{
    if (auth()->user()->role == 1) {
        return "Admin";
    }
...
function active_if_match_prefix($path)
{
    return Request::is($path . '*') ? 'active' : '';
}

function save_image($image, $directory = '/uploads/', $width = null, $height = null)
{
    $full_path = null;
    ...
    if (is_file($image)) {
        ...
        $save_path = public_path($directory) . $file_name . '.' . $image->extension();

        $img->save($save_path);

        $full_path = $directory . $file_name . '.' . $image->extension();
...
function uploadFile($file, $path)
{
    $image_path = null;
    if (is_file($file)) {
        $random_number = str_random(50);
        $image_path = $file->storeAs($path, $random_number . '.' . $file->extension(), 'public');
    }
    return $image_path;
}


function showStorageImage($path)
{
    if (\Illuminate\Support\Facades\Storage::disk('public')->exists($path)) {

        if (config('app.env') == 'production') {
            return \Illuminate\Support\Facades\Storage::url('app/public/' . $path);
        } else {
            return \Illuminate\Support\Facades\Storage::url($path);
        }
    } else {
        return asset('dash/plugins/ckeditor/plugins/placeholder/icons/placeholder.png');
    }
}

function localDays($id)
{
    foreach (trans('calender.days') as $key => $day) {
        if ($key == $id) {
            return $day;
        }
    }
...
function getLanguageJSON($lang)
{
    ...
    try {
        $jsonString = file_get_contents(resource_path('lang') . '/' . $lang . '/_intro.json');
    } catch (Exception $exception) {
        $jsonString = file_get_contents(resource_path('lang') . '/en/_intro.json');
    }
    $data = json_decode($jsonString, true);
    return $data;
}

function availableToday($doctor_id)
{
    ...
    $schedule = Schedule::where('day', $day)->where('doctor_id',$doctor_id)->with('scheduleDetails')->first();
    if($schedule){
...
function availableSevenDays($doctor_id)
{
    try {
    ...
    $schedule = Schedule::whereIn('day',$days_array)->where('doctor_id',$doctor_id)->with('scheduleDetails')->get();
    foreach($schedule as $key => $s)
    {
        $schedule_count = count($s->scheduleDetails);
        if($schedule_count > 0)
        {
            return true;
        }
...
function availableTomorrow($doctor_id)
{
    try {
        $day = Carbon::tomorrow()->format('l');
        } catch (\Exception $ex) {
